...
class LLaVA:
    ...
    def compute_steering_vector_matrix(self, dataset, safety_prefix, device='cuda'):
        ...
        for sample in tqdm(dataset, desc="计算转向向量"):
            ...
            input_ids = tokenizer_image_token(prompt, self.tokenizer, return_tensors='pt').unsqueeze(0).to(device)
            images_tensor, image_sizes = (None, None)
            if sample.get('img') is not None:
                images_tensor, image_sizes = self.prepare_imgs_tensor_both_cases(sample)
            with torch.no_grad():
                outputs_no_prefix = self.model(input_ids, images=images_tensor, image_sizes=image_sizes, output_hidden_states=True)
            safety_tokens = self.tokenizer(safety_prefix, return_tensors='pt').input_ids.to(device)
            input_ids_with_prefix = torch.cat([input_ids, safety_tokens[:, 1:]], dim=-1)
            with torch.no_grad():
                outputs_with_prefix = self.model(input_ids_with_prefix, images=images_tensor, image_sizes=image_sizes, output_hidden_states=True)
