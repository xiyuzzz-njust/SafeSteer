...
# --- 配置区域 ---
CONFIG = {
    "cfg_path": "/home/zengxiyu24/BAP/eval_configs/minigpt4_llama2_eval.yaml",
    ...
    # 输出文件保存目录
    "output_dir": "/home/zengxiyu24/HiddenDetect/train_classifier/hidden_states_minigpt_llama/fused_5_hidden_state_data_mix_pooled/"
}
...
def extract_and_save_states(label, samples_list, model_wrapper, output_path, k):
    ...
    for i, sample_info in enumerate(tqdm(samples_list, desc=f"正在处理 {label} 数据")):
        ...
        input_embeds = model_wrapper.prepare_inputs(sample_for_wrapper)  

        # 5. 执行图文前向传播
        with torch.no_grad():
            outputs = model_wrapper.model.llama_model(
        ...
        last_token_states_per_layer = [torch.mean(layer, dim=1).squeeze(0) for layer in last_k_layers]
...
def main():
    ...
    args = argparse.Namespace(cfg_path=CONFIG["cfg_path"], gpu_id=0, options=None)
    model_wrapper = MiniGPT4(args=args)
    print("模型封装类实例化成功！")
